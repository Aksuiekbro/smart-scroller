...
(() => {
  ...
  async function migrateSettings() {
    ...
    const patch = {
      ...
      blockedTopics: Array.isArray(existing.blockedTopics) ? existing.blockedTopics : [],
      filterMode: existing.filterMode === 'coach' ? 'coach' : 'focus',
      prehideUnknown: existing.prehideUnknown === true,
      hardHideOffTopic: existing.hardHideOffTopic === true,
      blockShortsSurfaces: existing.blockShortsSurfaces === true,
      autoSteer: existing.autoSteer === true,
      ...
      settingsPending = migrateSettings()
        .then((value) => {
          settingsCache = {
            ...
            blockedTopics: Array.isArray(value.blockedTopics) ? value.blockedTopics : [],
            filterMode: value.filterMode === 'coach' ? 'coach' : 'focus',
            prehideUnknown: value.prehideUnknown === true,
            hardHideOffTopic: value.hardHideOffTopic === true,
            blockShortsSurfaces: value.blockShortsSurfaces === true,
            autoSteer: value.autoSteer === true
          };
  ...
  function topicMatch(item, settings) {
    ...
    if (blockedHits.length) return { relevance: 'blocked', hits: blockedHits };
    const hits = matchTopics(settings.topics, haystack);
    if (hits.length) return { relevance: 'wanted', hits };
    return { relevance: settings.filterMode === 'coach' ? 'neutral' : 'unwanted', hits: [] };
  }
  ...
  async function review(feedItem = {}) {
    ...
    } else if (topic.relevance === 'blocked') {
      decision.action = 'blur';
      decision.reasons.unshift('This matches one of your avoid topics.');
  ...
  function expandHashtag(tag) {
    ...
    const expanded = value
      ...
      .replace(/[_-]+/g, ' ')
      .toLowerCase();
    return [value.toLowerCase(), expanded];
  }
  ...
  function metadataText(meta = {}) {
    return [
      ...
      meta.author,
      meta.description,
      ...(meta.hashtags || []).flatMap(expandHashtag)
    ].filter(Boolean).join(' ');
  }
  ...
  async function classify(meta = {}) {
    const settings = await loadSettings();
    const now = Date.now();
    if (!settings.enabled) return { onTopic: true, hits: [], reason: 'disabled' };
    if (settings.pauseUntil && now < settings.pauseUntil) {
      return { onTopic: true, hits: [], reason: 'paused' };
    }
    const hasTopicRules = settings.topics.length || settings.blockedTopics.length;
    if (!hasTopicRules && !settings.qualityEnabled) {
      return { onTopic: true, hits: [], reason: 'no-topics' };
    }

    const text = metadataText(meta);
    if (!normalizeForMatch(text)) return { onTopic: true, hits: [], reason: 'empty' };
    const topic = topicMatch({ text }, settings);
    if (topic.relevance === 'blocked') {
      return { onTopic: false, hits: topic.hits, reason: 'blocked' };
    }
    if (topic.relevance === 'unwanted') {
      return { onTopic: false, hits: [], reason: 'no-match' };
    }

    const decision = await review({
      platform: meta.platform || 'feed',
      surface: meta.surface || 'feed',
      author: meta.author,
      text,
      links: meta.links || [],
      media: meta.media || []
    ...
    if (decision.action === 'blur') {
      return {
        onTopic: false,
        hits: decision.labels.map((label) => ({ topic: 'Signal filter', keyword: label })),
        reason: 'low-signal',
        decision
      };
    }
    ...
    return {
      onTopic: true,
      hits: topic.hits,
      reason: !hasTopicRules ? 'no-topics' : topic.relevance === 'wanted' ? 'matched' : 'neutral',
      decision
    };
  }

  function siteEnabled(key) {
    return loadSettings().then((settings) => settings.sites?.[key] !== false);
  }
  ...
  async function addBlockedKeyword(keyword, topicName = 'Muted by SmartScroller') {
    ...
    const stored = await api.storage.sync.get('blockedTopics');
    const blockedTopics = Array.isArray(stored.blockedTopics) ? stored.blockedTopics : [];
    let topic = blockedTopics.find((entry) => entry.id === 'smart-muted');
    if (!topic) {
      topic = { id: 'smart-muted', name: topicName, keywords: [] };
      blockedTopics.push(topic);
    }
    if (!Array.isArray(topic.keywords)) topic.keywords = [];
    if (topic.keywords.includes(normalized)) return false;
    topic.keywords.push(normalized);
